...
const getAlbum = async () => {
    ...
    const albumsTemplate = albums.map(({id, title}) => `
        ...
        <tr>
            <td>${id}</td>
            <td>${title}</td>
            <td><a id="${id}" href="#album${id}">Click 2x</a> </td>
        </tr>
    `).join(" ");
...
//captura de click para ir para a galeria
document.addEventListener('dblclick', function(e){
    let selectedAlbum = e.target;
    ...
    getPhotos(selectedAlbum.id)
})

//captura do submit pra limpar tela e aplicar outro modo de visualização
